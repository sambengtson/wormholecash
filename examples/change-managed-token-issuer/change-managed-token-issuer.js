...
async function issueNewTokens() {
  try {
    ...
    let cashAddress2 = Wormhole.HDNode.toCashAddress(change2);

    // Create payload to change issuer
    let changeIssuer = await Wormhole.PayloadCreation.changeIssuer(216);
    ...
    let opReturn = await Wormhole.RawTransactions.opReturn(rawTx, changeIssuer);
