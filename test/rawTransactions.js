// let fixtures = require('./fixtures/PayloadCreation.json')
let chai = require('chai');
let assert = require('assert');
let wh = require('./../lib/Wormhole').default;
let Wormhole = new wh({
  restURL: "https://wormholecash-staging.herokuapp.com/v1/"
});

describe('#RawTransactions', () => {
  describe('#change', () => {
    it(`should change`, async () => {
      let change = await Wormhole.RawTransactions.change("", [{"txid":"6779a710fcd5f6fb0883ea3306360c3ad8c0a3c5de902768ec57ef3104e65eb1","vout":4,"scriptPubKey":"76a9147b25205fd98d462880a3e5b0541235831ae959e588ac","value":0.00068257}], "bchtest:qq2j9gp97gm9a6lwvhxc4zu28qvqm0x4j5e72v7ejg", 0.00003500, 1);
      assert.equal(change, '');
    });
  });

  describe('#input', () => {
    it(`should input`, async () => {
      let input = await Wormhole.RawTransactions.input("01000000000000000000", "b006729017df05eda586df9ad3f8ccfee5be340aadf88155b784d1fc0e8342ee", 0);
      assert.equal(input, '0100000001ee42830efcd184b75581f8ad0a34bee5feccf8d39adf86a5ed05df17907206b00000000000ffffffff0000000000');
    });
  });

  describe('#opReturn', () => {
    it(`should opReturn`, async () => {
      let opReturn = await Wormhole.RawTransactions.opReturn("01000000000000000000", "00000000000000020000000006dac2c0");
      assert.equal(opReturn, '0100000000010000000000000000166a140877686300000000000000020000000006dac2c000000000');
    });
  });

  describe('#reference', () => {
    it(`should reference`, async () => {
      let reference = await Wormhole.RawTransactions.reference("", "bchtest:qq2j9gp97gm9a6lwvhxc4zu28qvqm0x4j5e72v7ejg", 0.005);
      assert.equal(reference, '');
    });
  });

  describe('#decodeTransaction', () => {
    it(`should decodeTransaction`, async () => {
      let decodeTransaction = await Wormhole.RawTransactions.decodeTransaction("", [{"txid":"eeafb8c5b3be663f2ad0c00597a88f2896765b2ae30735791c7e476dce14af63","vout":1,"scriptPubKey":"76a9149084c0bd89289bc025d0264f7f23148fb683d56c88ac","value":0.0001123}]);
      assert.equal(decodeTransaction, 'Not a Master Protocol transaction');
    });
  });

  describe('#create', () => {
    it(`should create`, async () => {
      let create = await Wormhole.RawTransactions.create([{"txid":"myid","vout":0}], {});
      assert.equal(create, 'txid must be hexadecimal string (not \'myid\')');
    });
  });
});
