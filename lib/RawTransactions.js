...
var RawTransactions = function () {
  ...
  _createClass(RawTransactions, [{
    ...
    value: async function change(rawtx, prevTxs, destination, fee) {
      ...
      var path = path = this.restURL + 'rawTransactions/change';
      try {
        let response = await _axios2.default.post(path, {
          rawtx: rawtx,
          prevTxs: prevTxs,
    ...
    value: async function create(inputs) {
      ...
      try {
        var response = await _axios2.default.post(this.restURL + 'rawTransactions/create', {
          inputs: inputs,
          outputs: outputs
        });
        return response.data;
